...
parser.add_argument('--teacher_model_path',type=str,default="models/model_traintest_0.774")
...
for epoch in range(n_epochs):
    ...
    num_size_batches, num_value_batches, graph_batches, mat_batches, char_lengths, seg_batches, ori_batches = prepare_ro_batch(
        train_pairs, batch_size
    )

    print('-' * 20 + "每个epochs准备数据的时长: " + time_since(time.time() - start) + '-' * 20)
    print("epoch: {} ".format(epoch + 1))
    start = time.time()
    for idx in range(len(input_lengths)):
        loss = train_ro_tree(
            input_batches[idx], input_lengths[idx], output_batches[idx], output_lengths[idx],
            num_stack_batches[idx], num_size_batches[idx], generate_num_ids, encoder, predict, generate, merge, bert,
            encoder_optimizer, predict_optimizer, generate_optimizer, merge_optimizer, bert_optimizer, output_lang,
    ...
    if epoch % 2 == 0 or epoch > n_epochs - 5:
        ...
        for test_batch in test_pairs:
            ...
            batch_graph = get_single_example_graph(test_batch[0], test_batch[1], test_batch[7], test_batch[4],
                                                   test_batch[5])
            test_res = evaluate_ro_tree(test_batch[0], test_batch[1], generate_num_ids, encoder, predict, generate,
                                     merge, bert, output_lang, test_batch[5], batch_graph,
                                     test_batch[9], test_batch[8], beam_size=beam_size)
            # print(test_res,test_batch[2])
        ...
        if acc > best_acc:
            best_acc = acc
            if epoch>n_epochs*0.7:
                torch.save(encoder.state_dict(), "model_traintest/encoder_%.1f" % acc)
                torch.save(predict.state_dict(), "model_traintest/predict_%.1f"%acc)
                torch.save(generate.state_dict(), "model_traintest/generate_%.1f"%acc)
                torch.save(merge.state_dict(), "model_traintest/merge_%.1f"%acc)
                torch.save(bert.state_dict(), "model_traintest/bert_%.1f"%acc)
                best_acc_fold.append((equation_ac, value_ac, eval_total))
            # if best_acc >= 0.756:
            #     break
        print("best test_answer_acc", best_acc)
